...
@section('content')
    ...
    <form action="{{ route('revenue-triggers.index') }}" method="GET">
        <div class="row mb-3">
            ...
            <div class="col-md-1">
                <div class="col-md-1 mt-1">
                    <a href="{{ route('revenue-triggers.index') }}" class="btn btn-secondary">Clear</a>
                </div>
            </div>
    ...
    <div class="card-inner">
        <table class="datatable-init-export nowrap table" data-export-title="Export">
            ...
            @if(!empty($data['listings_without_booking_on_weekdays_seven_pm']))
                @foreach($data['listings_without_booking_on_weekdays_seven_pm'] as $lwbowsevenp)
                    
                    <tr>
                        <td>{{$sr}}</td>
                        <td>{{$lwbowsevenp->jsn_listing_name}}</td>
                        <td>Weekdays</td>
                        <td>Property unsold until 7PM KSA. Reduce Pricing by 10%</td>
            ...
            @if(!empty($data['listings_without_booking_on_weekends_six_pm']))
                @foreach($data['listings_without_booking_on_weekends_six_pm'] as $lwbowsixp)
                    ...
                    <tr>
                        ...
                        <td>Weekends</td>
                        <td>Property unsold until 6 PM KSA. Reduce Pricing by 5%</td>
            ...
            @if(!empty($data['listings_without_booking_on_weekends_eight_pm']))
                @foreach($data['listings_without_booking_on_weekends_eight_pm'] as $lwboweightp)
                    
                    <tr>
                        <td>{{$sr}}</td>
                        <td>{{$lwboweightp->jsn_listing_name}}</td>
                        <td>Weekends</td>
                        <td>Property unsold until 8 PM KSA. Reduce Pricing by 10%</td>
                        
                        @php $trlog = Illuminate\Support\Facades\DB::table('triggers_prices')->where(['listing_id'=>$lwboweightp->listing_id, 'trigger_type'=>$lwboweightp->days_type])->first(); @endphp
                        <td>{{!is_null($trlog) ? $trlog->trigger_logs : ''}}</td>
                        
                        <td>
                            <button title="Apply" type="button" class="btn btn-primary btn-sm" data-id="1" id="map" onclick="areYouSure('{{$lwboweightp->listing_id}}', '{{$lwboweightp->price_type}}', '{{$lwboweightp->days_type}}', {{$lwboweightp->percent}})">
                                <em class="icon ni ni-link"></em>
                            </button>
                            
                            @php
                            $tprice = route('revenue-triggers.revert_price', ['listing_id' => $lwboweightp->listing_id, 'days_type' => $lwboweightp->days_type]);
                            @endphp
                            <button title="Revert" type="button" class="btn btn-danger btn-sm" data-id="1" id="map" onclick="revertPrice('{{$tprice}}')">
                                <em class="icon ni ni-undo"></em>
                            </button>
            ...
            @if(!empty($data['listings_without_booking_on_weekends_ten_pm']))
                @foreach($data['listings_without_booking_on_weekends_ten_pm'] as $lwbowtenp)
                    
                    <tr>
                        <td>{{$sr}}</td>
                        <td>{{$lwbowtenp->jsn_listing_name}}</td>
                        <td>Weekends</td>
                        <td>Property unsold by 10 PM KSA. Reduce Pricing by 20%</td>
                        
                        @php $trlog = Illuminate\Support\Facades\DB::table('triggers_prices')->where(['listing_id'=>$lwbowtenp->listing_id, 'trigger_type'=>$lwbowtenp->days_type])->first(); @endphp
                        <td>{{!is_null($trlog) ? $trlog->trigger_logs : ''}}</td>
                        
                        <td>
                            <button title="Apply" type="button" class="btn btn-primary btn-sm" data-id="1" id="map" onclick="areYouSure('{{$lwbowtenp->listing_id}}', '{{$lwbowtenp->price_type}}', '{{$lwbowtenp->days_type}}', {{$lwbowtenp->percent}})">
                                <em class="icon ni ni-link"></em>
                            </button>
                            
                            @php
                            $tprice = route('revenue-triggers.revert_price', ['listing_id' => $lwbowtenp->listing_id, 'days_type' => $lwbowtenp->days_type]);
                            @endphp
                            <button title="Revert" type="button" class="btn btn-danger btn-sm" data-id="1" id="map" onclick="revertPrice('{{$tprice}}')">
                                <em class="icon ni ni-undo"></em>
                            </button>
