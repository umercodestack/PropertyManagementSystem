...
class RevenueTriggersController extends Controller
{
    ...
    public function AutoRevenueTrigger(){
        ...
        // echo $is_week_day . ' - ' . $ksaTime->hour;die;
        
        $data = [];
        if($is_weekend_day && in_array($ksaTime->hour, [20, 21])){ // 8 PM KSA time
            $data = $this->get_listings_without_bookings($eightPmKsaTime, $currentDate, 'reduce', 'weekends', 15, 1);
        }
        
        else if($is_weekend_day && in_array($ksaTime->hour, [18, 19])){ // 6 PM KSA time
            $data = $this->get_listings_without_bookings($sixPmKsaTime, $currentDate, 'reduce', 'weekends', 10, 1);
        }
        
        else if($is_week_day && $ksaTime->hour >= 15){ // 3 PM KSA time
            $data = $this->get_listings_without_bookings($threePmKsaTime, $currentDate, 'reduce', 'weekdays', 5, 1); // 0
        }
    ...
    private function autoTriggerSubmit($data){
        ...
        foreach($data as $record){
            ...
            $result = $this->update_price($record->listing_id, $property->ch_property_id, $rate_plan->ch_rate_plan_id, $record->price_type, $record->days_type, $record->percent, $update_days);
            return $result;
        }
    }
}
