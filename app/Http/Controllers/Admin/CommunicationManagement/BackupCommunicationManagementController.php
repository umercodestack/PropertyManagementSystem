    ...
    public function sendMessage(Request $request)
    {
        ...
        if ($response->successful()) {
            ...
            $threadMessage = ThreadMessage::create([
                ...
                'message_date' => Carbon::now()->addDay(),
...
//                );
  ...
  public function getNewRequests(Request $request) {
        ...
        if ($request->others == 'is_read') {
            $threads->where('is_read', 1);
        } elseif ($request->others == 'is_starred') {
            $threads->where('is_starred', 1);
        } elseif ($request->others == 'is_archived') {
            $threads->where('is_archived', 1);
        } elseif ($request->others == 'is_mute') {
            $threads->where('is_mute', 1);
        }
            ...
            if ($listing) {
                ...
                if ($thread) {
                    $finalResults[] = [
                        ...
                        'message_date' => Carbon::parse($bRequest->created_at)->subHour()->format('M d, Y, g:i A'),
        ...
        $oldThreads = Thread::whereNotIn('id', $threadIds)
        ...
        if ($request->others == 'is_read') {
            $oldThreads->where('is_read', 1);
        } elseif ($request->others == 'is_starred') {
            $oldThreads->where('is_starred', 1);
        } elseif ($request->others == 'is_archived') {
            $oldThreads->where('is_archived', 1);
        } elseif ($request->others == 'is_mute') {
            $oldThreads->where('is_mute', 1);
        }
    ...
    public function getBookingDetailsByThreadId($thread_id, $type)
    {
        ...
        if ($type == 'new_booking_request') {
            ...
            $bookingData['expires_in'] = Carbon::parse($booking['created_at'])->addDay();
      ...
      $isInquiry = false;
        ...
        if(!empty($thread->action_taken_at) && $thread->status == 'preapproval')
        {
            ...
            array_push($chats, [
                ...
                'message_date' => Carbon::parse($thread->action_taken_at),
                'guest_name' => $thread->name,
                'listing_name' => $thread->listing?->listing_name,
        ...
        foreach($thread->messages as $message)
        {
            array_push($chats, [
                ...
                'message_date' => $message->sender != 'channel' ? Carbon::parse($message->message_date) : $message->message_date,
                'guest_name' => $thread->name,
                'listing_name' => $thread->listing?->listing_name,
