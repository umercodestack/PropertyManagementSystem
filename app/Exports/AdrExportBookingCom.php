...
class AdrExportBookingCom implements FromCollection, WithTitle
{
    protected $data;
        ...
        foreach ($listings as $key => $items) {
            ...
            foreach ($period as $index => $item) {
                if (in_array($item->format('Y-m-d'), $calendar)) {
                    ...
                    ->where('booking_sources', 'booking_com')
                ...
                $otaBookingData = BookingOtasDetails::where('listing_id', $items->listing_id)
                    ...
                    ->whereJsonContains('booking_otas_json_details->attributes->ota_name', 'Booking.com')
