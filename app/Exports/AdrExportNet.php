...
class AdrExportNet implements FromCollection, WithTitle
{
    protected $data;
    protected $month;
    protected $year;
    ...
    public function __construct($data)
    {
        ...
        // Initialize DatePeriod and dates array once
    ...
    public function collection()
    {
        ...
        // Fetch calendar data for all listings
        ...
        $otaBookingData = BookingOtasDetails::whereIn('listing_id', array_merge($listings->pluck('listing_id')->toArray(), $listingRel->pluck('listing_id')->toArray()))
            ...
            ->select('id', 'listing_id', 'arrival_date', 'departure_date', 'booking_otas_json_details', 'amount', 'discount', 'promotion', 'cleaning_fee')
            ->get()
            ->groupBy('listing_id');
        ...
        // Build header
        $header = ['Apartment Title', 'Live Date', ...$this->dates];
        $occupancies = [$header];
        ...
        foreach ($listings as $listing) {
            ...
            $calendarDates = $listingCalendar->pluck('calender_date')->toArray();
            ...
            foreach ($this->dates as $date) {
                ...
                if ($directBooking) {
                    // Optionally exclude checkout date from daysCount
                    $daysCount = max(1, Carbon::parse($directBooking->booking_date_start)->diffInDays(Carbon::parse($directBooking->booking_date_end)->subDay()));
                ...
                if ($otaBooking) {
                    // Optionally exclude checkout date from daysCount
                    $daysCount = max(1, Carbon::parse($otaBooking->arrival_date)->diffInDays(Carbon::parse($otaBooking->departure_date)->subDay()));
                    $basePrice = $otaBooking->amount - $otaBooking->discount - $otaBooking->promotion;
                    ...
                    if ($listingRel) {
                        ...
                        if ($listingRelOtaBookings->isNotEmpty()) {
                            ...
                            if ($otaBooking) {
                                ...
                                $otaJson = json_decode($otaBooking->booking_otas_json_details);
                                $rawMessage = json_decode($otaJson->attributes->raw_message);

                                $basePrice = $rawMessage->reservation->listing_base_price_accurate;


                                // Adjust price for discounts and promotions
                                if (isset($rawMessage->reservation->pricing_rule_details)) {
                                    foreach ($rawMessage->reservation->pricing_rule_details as $rule) {
                                        $basePrice += abs($rule->amount_native); // Positive or negative adjustments
                                    }
                                }
                                if (isset($rawMessage->reservation->promotion_details)) {
                                    foreach ($rawMessage->reservation->promotion_details as $promo) {
                                        $basePrice += abs($promo->amount_native);
                                    }
                                }
                                if (isset($rawMessage->reservation->standard_fees_details)) {
                                    foreach ($rawMessage->reservation->standard_fees_details as $fee) {
                                        $basePrice += abs($fee->amount_native);
                                    }
                                }
                                $occupancy[$date] = round(($basePrice) / $daysCount, 2);
                                continue;
