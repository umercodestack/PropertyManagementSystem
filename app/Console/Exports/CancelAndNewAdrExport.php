...
class CancelAndNewAdrExport implements FromCollection, WithTitle
{
    protected $data;
    protected $month;
    protected $year;
    ...
    public function collection()
    {
        ...
        $bookingData = Bookings::whereIn('listing_id', $listings->pluck('id'))
            ...
            ->select('id', 'listing_id', 'booking_date_start', 'booking_date_end', 'per_night_price', 'cleaning_fee', 'booking_status')
            ->get()
            ->groupBy('listing_id');
        ...
        $otaBookingData = BookingOtasDetails::whereIn('listing_id', array_merge($listings->pluck('listing_id')->toArray(), $listingRel->pluck('listing_id')->toArray()))
            ...
            ->select('id', 'listing_id', 'arrival_date', 'departure_date', 'booking_otas_json_details', 'amount', 'discount', 'promotion', 'cleaning_fee', 'short_term_cleaning', 'status')
            ->get()
            ->groupBy('listing_id');
        // dd($otaBookingData);
        ...
        foreach ($listings as $listing) {
            ...
            foreach ($dates as $date) {
                ...
                $bookingDataCancelled = Bookings::where('listing_id', $listing->id)
                    ->where('booking_status', '=', 'cancelled')
                    ->whereDate('booking_date_start', '<=', $date)
                    ->whereDate('booking_date_end', '>=', $date)
                    ->select('id', 'listing_id', 'booking_date_start', 'booking_date_end', 'per_night_price', 'cleaning_fee', 'booking_status')
                    ->first();
                if ($bookingDataCancelled && !$directBooking) {
                    $daysCountCancelled = max(1, Carbon::parse($bookingDataCancelled->booking_date_start)->diffInDays($bookingDataCancelled->booking_date_end));
                    $occupancy[$date] = round($bookingDataCancelled->per_night_price + ($bookingDataCancelled->cleaning_fee / $daysCountCancelled), 2);
                    continue;
                }
                if ($directBooking) {
                    // dd($date);
                    $bookingDataCancelled = Bookings::where('listing_id', $directBooking->listing_id)
                        ->where('booking_status', '=', 'cancelled')
                        ->whereDate('booking_date_start', '<=', $date)
                        ->whereDate('booking_date_end', '>=', $date)
                        ->select('id', 'listing_id', 'booking_date_start', 'booking_date_end', 'per_night_price', 'cleaning_fee', 'booking_status')
                        ->first();
                    if ($bookingDataCancelled) {
                        $daysCount = max(1, Carbon::parse($directBooking->booking_date_start)->diffInDays($directBooking->booking_date_end));
                        $daysCountCancelled = max(1, Carbon::parse($bookingDataCancelled->booking_date_start)->diffInDays($bookingDataCancelled->booking_date_end));
                        $occupancy[$date] = round($directBooking->per_night_price + ($directBooking->cleaning_fee / $daysCount), 2) + round($bookingDataCancelled->per_night_price + ($bookingDataCancelled->cleaning_fee / $daysCountCancelled), 2);
                    } else {
                        $daysCount = max(1, Carbon::parse($directBooking->booking_date_start)->diffInDays($directBooking->booking_date_end));
                ...
                $cancellet0taBookingData = BookingOtasDetails::where('listing_id', $listing->listing_id)
                    ->where('status', '=', 'cancelled')
                    ->whereDate('arrival_date', '<=', $date)
                    ->whereDate('departure_date', '>=', $date)
                    ->select('id', 'listing_id', 'arrival_date', 'departure_date', 'booking_otas_json_details', 'amount', 'discount', 'promotion', 'cleaning_fee', 'short_term_cleaning', 'status')
                    ->first();
                if ($cancellet0taBookingData && !$otaBooking) {
                    // dd($cancellet0taBookingData, $listing);
                    $daysCountCancelled = max(1, Carbon::parse($cancellet0taBookingData->arrival_date)->diffInDays($cancellet0taBookingData->departure_date));
                    $basePriceCancelled = $cancellet0taBookingData->amount + $cancellet0taBookingData->cleaning_fee;
                    $occupancy[$date] =
                        ($basePriceCancelled > 0 ? round($daysCountCancelled / $basePriceCancelled, 2) : 0);
                    continue;                  // dd($cancellet0taBookingData);
                }
                ...
                if ($otaBooking) {
                    ...
                    // }
                    $cancellet0taBookingData = BookingOtasDetails::where('listing_id', $otaBooking->listing_id)
                        ->where('status', '=', 'cancelled')
                        ->whereDate('arrival_date', '<=', $date)
                        ->whereDate('departure_date', '>=', $date)
                        ->select('id', 'listing_id', 'arrival_date', 'departure_date', 'booking_otas_json_details', 'amount', 'discount', 'promotion', 'cleaning_fee', 'short_term_cleaning', 'status')
                        ->first();
                    if ($cancellet0taBookingData) {
                        $daysCountCancelled = max(1, Carbon::parse($cancellet0taBookingData->arrival_date)->diffInDays($cancellet0taBookingData->departure_date));
                        $basePriceCancelled = $cancellet0taBookingData->amount + $cancellet0taBookingData->cleaning_fee;
                        $occupancy[$date] =
                            ($daysCount > 0 ? round($basePrice / $daysCount, 2) : 0)
                            + ($basePriceCancelled > 0 ? round($daysCountCancelled / $basePriceCancelled, 2) : 0);                        // dd($cancellet0taBookingData);
                    } else {
                ...
                if ($listingRel) {
                    ...
                    if ($listingRel) {
                        ...
                        if ($listingRelOtaBookings->isNotEmpty()) {
                            ...
                            // dd($listingRel->listing_id);
                            $cancellet0taBookingData = BookingOtasDetails::where('listing_id', $listingRel->listing_id)
                                ->where('status', '=', 'cancelled')
                                ->whereDate('arrival_date', '<=', $date)
                                ->whereDate('departure_date', '>=', $date)
                                ->select('id', 'listing_id', 'arrival_date', 'departure_date', 'booking_otas_json_details', 'amount', 'discount', 'promotion', 'cleaning_fee', 'short_term_cleaning', 'status')
